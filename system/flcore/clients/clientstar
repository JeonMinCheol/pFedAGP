...
class clientstar(Client):
    def __init__(self, args, id, train_samples, test_samples, **kwargs):
        ...
        self.lambda_proto_pull = getattr(args, "lambda_proto_pull", 0.0)  # CE에 더하는 가중치 (0.3)
        self.lambda_proto_ce   = getattr(args, "lambda_proto_ce",   0.0)  # (옵션) 프로토타입 분류 보조 로스 (0.1)
    ...
    def train(self):
                ...
                if hasattr(self, "local_style") and self.local_style:
                    ...
                    for yy in y.tolist():
                        ...
                        if isinstance(s, torch.Tensor):
                            ...
                            style_batch.append(torch.zeros_like(reps[0]))
                    style_batch = torch.stack(style_batch, dim=0)  # [B, D]
                    reps = self.film(reps, style_batch)  # personalized feature modulation
        ...
        # print(f"Client {self.id}, number of batch: {len(trainloader)}, time: {time.time() - start_time}")
