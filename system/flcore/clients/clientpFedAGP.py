...
class ClientPrototypeGenerator(nn.Module):
    def __init__(self, model: nn.Module, dropout: float = 0.4):
        super().__init__()
            ...
            for ii, (x, y) in enumerate(trainloader):
                ...
                if self.global_protos is not None and self.protos is not None:
                    ...
                    if self.mask.any():
                        ...
                        proto_valid = torch.stack([
                            self.protos[int(lbl)] for i, lbl in enumerate(y) if self.mask[i]
                        ], dim=0).squeeze(1).to(self.device).detach()  # [B', D]

                        # ✅ 차원 맞춤
                        features = torch.stack([rep_valid, proto_valid], dim=1)  # [B', 2, D]

                        supcon_loss = self.supcon_loss(features, labels_valid)
                
                total_loss = ce_loss * self.ce_lambda + supcon_loss
                ...
                self.protos = self.proto_gen(batch_emb)
