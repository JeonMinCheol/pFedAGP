...
class FashionCNNModel(nn.Module):
    def __init__(self, num_classes= 10):
        ...
        self.backbone = nn.Sequential(
            ...
            # GAP 7×7 → 1×1   → 128-dim 벡터
            nn.AdaptiveAvgPool2d(1),
            nn.Flatten(),

            # 128 → 256 → 64 (임베딩 차원 축소)
            nn.Linear(128, 256),
            nn.ReLU(inplace=True),
            nn.Dropout(0.5),
            nn.Linear(256, 64),
        ...
        #self.class_fc = self.head[-1] 
        #self.fc3 = self.head[-1]      

    # ──────────────────────────────────────
    def forward(self, x):
        emb = self.backbone(x)   # 64-dim 공통 임베딩
        out = self.fc(emb)     # num_classes 로짓
        return out, emb

    @torch.no_grad()
    def extract_feature_embedding(self, x):
        """64-차원 공통 임베딩만 반환"""
